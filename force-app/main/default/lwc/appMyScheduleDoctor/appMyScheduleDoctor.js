...
export default class AppMyScheduleDoctor extends LightningElement {
    @api modoHome = false;
    ...
    async carregarAgendaReal() {
        ...
        try {
            const resultado = await listarAgendaMedicoLogado({ dateStart, dateEnd });    
    ...
    connectedCallback() {
        ...
        this.updateCardsPorTamanho();

        // Atualiza responsivamente
        window.addEventListener('resize', this.updateCardsPorTamanho.bind(this));
    }      
    ...
    get consultasParaExibir() {
        if (this.modoHome) {
            ...
            return this.filteredAgenda.slice(0, this.maxCardsVisiveis);
        }
        return [];
    }  
    ...
    updateCardsPorTamanho() {
        const larguraTela = window.innerWidth;

        if (larguraTela >= 1400) {
            this.maxCardsVisiveis = 6;
        } else if (larguraTela >= 1000) {
            this.maxCardsVisiveis = 4;
        } else if (larguraTela >= 768) {
            this.maxCardsVisiveis = 3;
        } else {
            this.maxCardsVisiveis = 3;
        }
    }
